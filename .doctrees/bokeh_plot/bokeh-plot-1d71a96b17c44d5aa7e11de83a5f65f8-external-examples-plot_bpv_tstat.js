(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("7f99e3d5-ee1a-43f5-8820-7f9316bd4beb");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '7f99e3d5-ee1a-43f5-8820-7f9316bd4beb' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.3.1.min.js": "YF85VygJKMVnHE+lLv2AM93Vbstr0yo2TbIu5v8se5Rq3UQAUmcuh4aaJwNlpKwa", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.1.min.js": "KKuas3gevv3PvrlkyCMzffFeaMq5we/a2QsP5AUoS3mJ0jmaCL7jirFJN3GoE/lM", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.1.min.js": "MK/uFc3YT18pkvvXRl66tTHjP0/dxoSH2e/eiNMFIguKlun2+WVqaPTWmUy/zvh4"};
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.3.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.1.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.1.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"45f6e2de-cab3-41a3-9291-8028787cbcea":{"defs":[],"roots":{"references":[{"attributes":{"source":{"id":"14922"}},"id":"14926","type":"CDSView"},{"attributes":{"formatter":{"id":"14912"},"major_label_policy":{"id":"14911"},"ticker":{"id":"14870"}},"id":"14869","type":"LinearAxis"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"14924","type":"Circle"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14897","type":"Line"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"14886","type":"PolyAnnotation"},{"attributes":{},"id":"14867","type":"LinearScale"},{"attributes":{"data":{},"selected":{"id":"14919"},"selection_policy":{"id":"14918"}},"id":"14901","type":"ColumnDataSource"},{"attributes":{},"id":"14874","type":"BasicTicker"},{"attributes":{},"id":"14943","type":"Selection"},{"attributes":{"toolbars":[{"id":"14887"}],"tools":[{"id":"14877"},{"id":"14878"},{"id":"14879"},{"id":"14880"},{"id":"14881"},{"id":"14882"},{"id":"14883"},{"id":"14884"}]},"id":"14946","type":"ProxyToolbar"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14902","type":"Line"},{"attributes":{"axis":{"id":"14869"},"ticker":null},"id":"14872","type":"Grid"},{"attributes":{},"id":"14912","type":"BasicTickFormatter"},{"attributes":{"axis":{"id":"14873"},"dimension":1,"ticker":null},"id":"14876","type":"Grid"},{"attributes":{"text":"y / y","text_font_size":"15pt"},"id":"14927","type":"Title"},{"attributes":{"overlay":{"id":"14886"}},"id":"14881","type":"LassoSelectTool"},{"attributes":{"data_source":{"id":"14896"},"glyph":{"id":"14897"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14898"},"view":{"id":"14900"}},"id":"14899","type":"GlyphRenderer"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"14885","type":"BoxAnnotation"},{"attributes":{"callback":null},"id":"14884","type":"HoverTool"},{"attributes":{},"id":"14861","type":"DataRange1d"},{"attributes":{"data":{},"selected":{"id":"14943"},"selection_policy":{"id":"14942"}},"id":"14922","type":"ColumnDataSource"},{"attributes":{},"id":"14909","type":"BasicTickFormatter"},{"attributes":{},"id":"14918","type":"UnionRenderers"},{"attributes":{"source":{"id":"14896"}},"id":"14900","type":"CDSView"},{"attributes":{"below":[{"id":"14869"}],"center":[{"id":"14872"},{"id":"14876"},{"id":"14920"}],"height":500,"left":[{"id":"14873"}],"output_backend":"webgl","renderers":[{"id":"14899"},{"id":"14904"},{"id":"14925"}],"title":{"id":"14927"},"toolbar":{"id":"14887"},"toolbar_location":null,"width":500,"x_range":{"id":"14861"},"x_scale":{"id":"14865"},"y_range":{"id":"14863"},"y_scale":{"id":"14867"}},"id":"14860","subtype":"Figure","type":"Plot"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14903","type":"Line"},{"attributes":{"source":{"id":"14901"}},"id":"14905","type":"CDSView"},{"attributes":{},"id":"14917","type":"Selection"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"14904"}]},"id":"14921","type":"LegendItem"},{"attributes":{},"id":"14883","type":"SaveTool"},{"attributes":{},"id":"14908","type":"AllLabels"},{"attributes":{},"id":"14916","type":"UnionRenderers"},{"attributes":{"data_source":{"id":"14922"},"glyph":{"id":"14923"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14924"},"view":{"id":"14926"}},"id":"14925","type":"GlyphRenderer"},{"attributes":{"data_source":{"id":"14901"},"glyph":{"id":"14902"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14903"},"view":{"id":"14905"}},"id":"14904","type":"GlyphRenderer"},{"attributes":{},"id":"14919","type":"Selection"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"14917"},"selection_policy":{"id":"14916"}},"id":"14896","type":"ColumnDataSource"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"14923","type":"Circle"},{"attributes":{},"id":"14942","type":"UnionRenderers"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14898","type":"Line"},{"attributes":{},"id":"14880","type":"WheelZoomTool"},{"attributes":{"active_multi":null,"tools":[{"id":"14877"},{"id":"14878"},{"id":"14879"},{"id":"14880"},{"id":"14881"},{"id":"14882"},{"id":"14883"},{"id":"14884"}]},"id":"14887","type":"Toolbar"},{"attributes":{"children":[[{"id":"14860"},0,0]]},"id":"14945","type":"GridBox"},{"attributes":{},"id":"14870","type":"BasicTicker"},{"attributes":{"children":[{"id":"14947"},{"id":"14945"}]},"id":"14948","type":"Column"},{"attributes":{},"id":"14911","type":"AllLabels"},{"attributes":{"overlay":{"id":"14885"}},"id":"14879","type":"BoxZoomTool"},{"attributes":{},"id":"14878","type":"PanTool"},{"attributes":{},"id":"14865","type":"LinearScale"},{"attributes":{"formatter":{"id":"14909"},"major_label_policy":{"id":"14908"},"ticker":{"id":"14874"}},"id":"14873","type":"LinearAxis"},{"attributes":{"toolbar":{"id":"14946"},"toolbar_location":"above"},"id":"14947","type":"ToolbarBox"},{"attributes":{},"id":"14877","type":"ResetTool"},{"attributes":{},"id":"14863","type":"DataRange1d"},{"attributes":{"items":[{"id":"14921"}]},"id":"14920","type":"Legend"},{"attributes":{},"id":"14882","type":"UndoTool"}],"root_ids":["14948"]},"title":"Bokeh Application","version":"2.3.1"}}';
                  var render_items = [{"docid":"45f6e2de-cab3-41a3-9291-8028787cbcea","root_ids":["14948"],"roots":{"14948":"7f99e3d5-ee1a-43f5-8820-7f9316bd4beb"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();